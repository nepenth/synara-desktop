...
//!
//! Reuses the same homeserver URL plausibility rules as
//! [`crate::matrix::store::AccountIdentity`] so path/key isolation and auth
//! discovery agree on what counts as a valid homeserver base URL.
...
pub enum DiscoveryInput {
    ...
    /// Matrix server name only (`example.org` or `example.org:8448`) — requires
    /// well-known discovery via [`super::discovery::DiscoveryTransport`].
    ServerName(String),
...
fn is_plausible_homeserver_url(url: &str) -> bool {
    ...
    let has_scheme = (lower.starts_with("https://") || lower.starts_with("http://"))
        && url.len() > "https://".len();
    if !has_scheme || url.contains("..") {
        return false;
    }
    // Require a non-empty host after scheme.
    let after = if lower.starts_with("https://") {
        &url["https://".len()..]
    } else {
        &url["http://".len()..]
    };
