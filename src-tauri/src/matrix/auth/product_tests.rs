...
fn room_publish_join_rule_snapshot_registers_one_native_read_owner() {
    ...
    let core_live = include_str!("../../../../crates/synara-core/src/app/room_profile/live.rs");
    ...
    let generated_write =
        include_str!("../../../permissions/autogenerated/matrix_room_set_join_rule.toml");
    let schemas = [
    ...
    for command in [
        ...
        let permission = if command == "matrix_room_set_join_rule" {
            generated_write
        } else {
            generated
        };
        assert!(permission.contains(&format!("allow-{capability_id}")));
        assert!(permission.contains(&format!("deny-{capability_id}")));
    ...
    let write_command = product
        .split("pub async fn matrix_room_set_join_rule")
        .nth(1)
        .expect("join-rule write command")
        .split("#[tauri::command]")
        .next()
        .expect("join-rule write command body");
    assert!(write_command.contains("crate::bridge::join_rule_snapshot::set_join_rule"));
    assert!(write_command.contains("core: State<'_, Arc<synara_core::Core>>"));
    assert!(!write_command.contains("update_join_rule"));
    assert!(!write_command.contains("send_state_event"));
    ...
    assert!(core_live.contains("update_join_rule"));
    assert!(core_live.contains("pub async fn set_join_rule"));
}

#[test]
fn inbox_and_approvals_list_ipc_is_explicit_registered_on_every_desktop_schema() {
    let product = PRODUCT_SOURCE;
    let lib = include_str!("../../lib.rs");
    let build = include_str!("../../../build.rs");
    let capability = include_str!("../../../capabilities/main.json");
    ...
    let generated = [
        include_str!("../../../permissions/autogenerated/matrix_agent_approvals_list.toml"),
        include_str!("../../../permissions/autogenerated/matrix_inbox_notifications.toml"),
    ];
    for (index, command) in ["matrix_agent_approvals_list", "matrix_inbox_notifications"]
        .into_iter()
        .enumerate()
    {
        ...
        assert!(generated[index].contains(command));
        assert!(generated[index].contains(&format!("allow-{permission}")));
        assert!(generated[index].contains(&format!("deny-{permission}")));
        for schema in schemas {
            assert!(
                schema.contains(command),
                "{command} missing from a generated capability schema"
            );
        }
    }
}
...
#[test]
fn user_directory_search_routes_through_core_without_desktop_client_io() {
    let product = PRODUCT_SOURCE;
    let command = product
        .split("pub async fn matrix_user_directory_search")
        .nth(1)
        .expect("user directory search command");
    let command = command
        .split("#[tauri::command]")
        .next()
        .expect("user directory search command body");
    assert!(command.contains("crate::bridge::user_directory::user_directory_search"));
    assert!(command.contains("core: State<'_, Arc<synara_core::Core>>"));
    assert!(!command.contains("active.client"));
    assert!(!command.contains("search_users"));
}

#[test]
fn message_search_routes_through_core_without_desktop_client_io() {
    let product = PRODUCT_SOURCE;
    let command = product
        .split("pub async fn matrix_message_search")
        .nth(1)
        .expect("message search command");
    let command = command
        .split("#[tauri::command]")
        .next()
        .expect("message search command body");
    assert!(command.contains("crate::bridge::message_search::message_search"));
    assert!(command.contains("core: State<'_, Arc<synara_core::Core>>"));
    assert!(!command.contains("active.client"));
    assert!(!command.contains("search_messages"));
    assert!(!command.contains("search_index"));
    assert!(!command.contains("dual_backend"));
    assert!(!command.contains("mx.search"));
}
...
#[test]
fn v_auth_sync_recover_restarts_live_owner_without_rebuild() {
    let recover = AUTH_PRODUCT_COMMANDS_SOURCE
        .split("pub async fn matrix_sync_recover")
        .nth(1)
        .and_then(|source| source.split("#[tauri::command]").next())
        .expect("matrix_sync_recover body");
    assert!(recover.contains("recover_sync_after_wake"));
    assert!(!recover.contains("start_sync_owner"));
    assert!(!recover.contains("access_token"));
    ...
    let build = include_str!("../../../build.rs");
    let capability = include_str!("../../../capabilities/main.json");
    let permission = include_str!("../../../permissions/autogenerated/matrix_sync_recover.toml");
    assert!(lib.contains("matrix_sync_recover"));
    assert!(lib.contains("spawn_suspend_resume_watch"));
    assert!(build.contains("\"matrix_sync_recover\""));
    assert!(capability.contains("allow-matrix-sync-recover"));
    assert!(permission.contains("matrix_sync_recover"));
}
