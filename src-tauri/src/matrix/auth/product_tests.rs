...
const PRODUCT_SOURCE: &str = concat!(
    ...
    include_str!("../rtc_transports/product_commands.rs"),
    include_str!("../user_status/product_commands.rs"),
...
#[test]
fn v_rtc_transports_registers_the_frozen_native_surface() {
    let rtc_source = include_str!("../rtc_transports/product_commands.rs");
    let lib_source = include_str!("../../lib.rs");
    let build = include_str!("../../../build.rs");
    let capability = include_str!("../../../capabilities/main.json");
    let snapshot_permission =
        include_str!("../../../permissions/autogenerated/matrix_rtc_transports_snapshot.toml");
    let refresh_permission =
        include_str!("../../../permissions/autogenerated/matrix_rtc_transports_refresh.toml");
    for command in [
        "matrix_rtc_transports_snapshot",
        "matrix_rtc_transports_refresh",
    ] {
        assert!(rtc_source.contains(command));
        assert!(lib_source.contains(command));
        assert!(build.contains(command));
    }
    assert!(capability.contains("allow-matrix-rtc-transports-snapshot"));
...
#[test]
fn v_user_status_registers_the_frozen_native_surface() {
    let status_source = include_str!("../user_status/product_commands.rs");
    let lib_source = include_str!("../../lib.rs");
    let build = include_str!("../../../build.rs");
    let capability = include_str!("../../../capabilities/main.json");
    let snapshot_permission =
        include_str!("../../../permissions/autogenerated/matrix_user_status_snapshot.toml");
    let set_permission =
        include_str!("../../../permissions/autogenerated/matrix_user_status_set.toml");
    let clear_permission =
        include_str!("../../../permissions/autogenerated/matrix_user_status_clear.toml");
    for command in [
        "matrix_user_status_snapshot",
        "matrix_user_status_set",
        "matrix_user_status_clear",
    ] {
        assert!(status_source.contains(command));
        assert!(lib_source.contains(command));
        assert!(build.contains(command));
    }
    assert!(capability.contains("allow-matrix-user-status-snapshot"));
    assert!(capability.contains("allow-matrix-user-status-set"));
    assert!(capability.contains("allow-matrix-user-status-clear"));
    assert!(snapshot_permission.contains("matrix_user_status_snapshot"));
    assert!(set_permission.contains("matrix_user_status_set"));
    assert!(clear_permission.contains("matrix_user_status_clear"));
    let schemas = [
        include_str!("../../../gen/schemas/desktop-schema.json"),
        include_str!("../../../gen/schemas/linux-schema.json"),
        include_str!("../../../gen/schemas/macOS-schema.json"),
    ...
    for schema in schemas {
        assert!(schema.contains("allow-matrix-user-status-snapshot"));
        assert!(schema.contains("allow-matrix-user-status-set"));
        assert!(schema.contains("allow-matrix-user-status-clear"));
    }
...
#[test]
fn encrypted_state_leftover_commands_register_native_owners() {
    let product_profile = include_str!("../room_profile/product_commands.rs");
    let product_ops = include_str!("../room_ops/product_commands.rs");
    let lib = include_str!("../../lib.rs");
    let build = include_str!("../../../build.rs");
    let capability = include_str!("../../../capabilities/main.json");
    let send = include_str!("../../../permissions/autogenerated/matrix_send_state_event.toml");
    let enable =
        include_str!("../../../permissions/autogenerated/matrix_enable_room_encrypted_state.toml");
    let setting = include_str!(
        "../../../permissions/autogenerated/matrix_set_encrypted_state_events_setting.toml"
    );
...
#[test]
fn room_retention_command_is_registered_and_read_only() {
    let product = PRODUCT_SOURCE;
    let lib = include_str!("../../lib.rs");
    let build = include_str!("../../../build.rs");
    let capability = include_str!("../../../capabilities/main.json");
    assert!(product.contains("pub async fn matrix_room_retention"));
    assert!(lib.contains("matrix_room_retention"));
    assert!(build.contains("\"matrix_room_retention\""));
    assert!(capability.contains("allow-matrix-room-retention"));
    assert!(
        include_str!("../../../permissions/autogenerated/matrix_room_retention.toml")
            .contains("matrix_room_retention")
    );
    for schema in [
        include_str!("../../../gen/schemas/desktop-schema.json"),
        include_str!("../../../gen/schemas/linux-schema.json"),
        ...
        assert!(schema.contains("matrix_room_retention"));
        assert!(schema.contains("allow-matrix-room-retention"));
        assert!(schema.contains("deny-matrix-room-retention"));
    }
    let command = product
        .split("pub async fn matrix_room_retention")
        .nth(1)
        .expect("room retention command")
        .split("#[tauri::command]")
        .next()
        .expect("room retention command body");
    assert!(command.contains("crate::bridge::room_retention::room_retention"));
    assert!(command.contains("core: State<'_, Arc<synara_core::Core>>"));
    assert!(!command.contains("update_room_retention"));
...
#[test]
fn media_preview_command_is_registered_and_read_only() {
    let product = PRODUCT_SOURCE;
    let lib = include_str!("../../lib.rs");
    let build = include_str!("../../../build.rs");
    let capability = include_str!("../../../capabilities/main.json");
    assert!(product.contains("pub async fn matrix_media_preview"));
    assert!(lib.contains("matrix_media_preview"));
    assert!(build.contains("\"matrix_media_preview\""));
    assert!(capability.contains("allow-matrix-media-preview"));
    assert!(
        include_str!("../../../permissions/autogenerated/matrix_media_preview.toml")
            .contains("matrix_media_preview")
    );
    for schema in [
        include_str!("../../../gen/schemas/desktop-schema.json"),
        include_str!("../../../gen/schemas/linux-schema.json"),
        ...
        assert!(schema.contains("matrix_media_preview"));
        assert!(schema.contains("allow-matrix-media-preview"));
        assert!(schema.contains("deny-matrix-media-preview"));
    }
    let command = product
        .split("pub async fn matrix_media_preview")
        .nth(1)
        .expect("media preview command")
        .split("#[tauri::command]")
        .next()
        .expect("media preview command body");
    assert!(command.contains("crate::bridge::media_preview::media_preview"));
    assert!(command.contains("core: State<'_, Arc<synara_core::Core>>"));
