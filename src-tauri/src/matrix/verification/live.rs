...
impl NativeVerificationOwner {
        ...
        let (request, other_device_id) = match device_id {
            Some(device_id) => {
                ...
                let device = client
                    ...
                    .await
                    .map_err(|_| sdk_error("v-crypto.1-device-query-failed"))?
                    .ok_or_else(|| {
                        verification_error(
                            "NotFound",
                ...
                let request = device
                    ...
                    .await
                    .map_err(|_| sdk_error("v-crypto.1-device-request-failed"))?;
            ...
            None => {
                let identity = client
                    ...
                    .await
                    .map_err(|_| sdk_error("v-crypto.1-identity-query-failed"))?
                    .ok_or_else(|| {
                        verification_error(
                            "Unsupported",
                ...
                let request = identity
                    .request_verification_with_methods(vec![VerificationMethod::SasV1])
                    .await
                    .map_err(|_| sdk_error("v-crypto.1-own-request-failed"))?;
                (request, None)
            }
        };
    ...
    pub async fn accept(
        ...
        request
            ...
            .await
            .map_err(|_| sdk_error("v-crypto.1-accept-failed"))?;
    ...
    pub async fn begin_sas(
        ...
        let sas = match request.state() {
            VerificationRequestState::Ready { .. } => request
                .start_sas()
                .await
                .map_err(|_| sdk_error("v-crypto.1-sas-start-failed"))?
                .ok_or_else(|| sdk_error("v-crypto.1-sas-start-unavailable"))?,
            VerificationRequestState::Transitioned {
                ...
                if !sas.we_started() {
                    sas.accept()
                        .await
                        .map_err(|_| sdk_error("v-crypto.1-sas-accept-failed"))?;
                }
                sas
            }
            _ => {
    ...
    pub async fn confirm(
        ...
        sas.confirm()
            .await
            .map_err(|_| sdk_error("v-crypto.1-confirm-failed"))?;
        let mut registry = self.registry.lock().await;
        let managed = registry
            .requests
            .get_mut(flow_id)
    ...
    pub async fn mismatch(
        ...
        sas.mismatch()
            .await
            .map_err(|_| sdk_error("v-crypto.1-mismatch-failed"))?;
        let mut registry = self.registry.lock().await;
        let managed = registry
            .requests
            .get_mut(flow_id)
    ...
    pub async fn cancel(
        ...
        if let Some(sas) = sas {
            sas.cancel()
                .await
                .map_err(|_| sdk_error("v-crypto.1-sas-cancel-failed"))?;
        } else {
            request
                .cancel()
                .await
                .map_err(|_| sdk_error("v-crypto.1-request-cancel-failed"))?;
        }
        self.snapshot(flow_id).await
    }

    pub async fn dismiss(&self, flow_id: &str) -> Result<(), MatrixAuthCommandError> {
        let mut registry = self.registry.lock().await;
        let managed = registry
            .requests
            .get_mut(flow_id)
    ...
    async fn request(&self, flow_id: &str) -> Result<VerificationRequest, MatrixAuthCommandError> {
        let registry = self.registry.lock().await;
        registry
            .requests
            .get(flow_id)
...
fn phase_rank(phase: NativeVerificationPhase) -> u8 {
    match phase {
...
fn unknown_flow_error() -> MatrixAuthCommandError {
    verification_error(
        "NotFound",
...
fn sdk_error(diagnostic_id: &'static str) -> MatrixAuthCommandError {
    verification_error(
        "Unknown",
