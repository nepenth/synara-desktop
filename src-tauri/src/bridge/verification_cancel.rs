//! Desktop bridge for `matrix_verification_cancel` through `Core::command`.
//!
...
//! existing Tauri error shape. React still invokes `matrix_verification_cancel`.
...
pub(crate) async fn verification_cancel(
    core: &Core,
    flow_id: String,
) -> Result<NativeVerificationRequest, MatrixAuthCommandError> {
    let response = core
        .command(CommandEnvelope {
            command: VERIFICATION_CANCEL_COMMAND.to_owned(),
            session_generation: READ_ONLY_SESSION_GENERATION,
            request_id: None,
            payload: serde_json::json!({ "flowId": flow_id }),
        })
        .await
        .map_err(map_verification_cancel_core_error)?;
    serde_json::from_value(response.payload).map_err(|_| verification_cancel_response_error())
}

fn map_verification_cancel_core_error(error: MatrixIpcError) -> MatrixAuthCommandError {
    match error.category {
        ...
        _ => match error.diagnostic_id.as_deref() {
            Some("v-crypto.1-sas-cancel-failed") => {
                map_verification_error("v-crypto.1-sas-cancel-failed")
            }
            Some("v-crypto.1-request-cancel-failed") => {
                map_verification_error("v-crypto.1-request-cancel-failed")
            }
            _ => map_verification_error("v-crypto.1-request-cancel-failed"),
        },
    }
}

fn verification_cancel_response_error() -> MatrixAuthCommandError {
    map_verification_error("v-crypto.1-request-cancel-failed")
}
