...
| `npm run test:modernization` | 273/273 pass |
...
| Feature | Status | Location |
|---------|--------|----------|
| Room timeline (virtualized, ~2945 LOC) | Full | `features/room/RoomTimeline.tsx` |
...
| Monolithic files | Medium | `RoomTimeline.tsx` (2945 LOC), `desktop.rs` (5101 LOC) |
...
check:repo-layout → check:versions → check:matrix-boundaries
  → cargo check/test → typecheck:modernization → test:modernization (273)
  → eslint → prettier
```
