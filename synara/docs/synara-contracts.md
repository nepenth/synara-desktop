...
Status: local pre-iOS contract inventory complete. This inventory covers the
portable contracts needed before packaged macOS/Linux validation and native iOS
scaffolding. It does not replace future Swift conformance tests.
...
| Room notes                | [Room Notes](./synara-room-notes-contract.md)       | `docs/contracts/synara-room-notes-content.schema.json`                                                 | `src/app/utils/roomNotes.ts`            | Future native room notes service | Writers emit canonical v1; readers normalize malformed or oversized items.        |
...
## Local Pre-iOS Completion State
