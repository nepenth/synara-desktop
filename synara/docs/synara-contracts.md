...
| Agent approval history    | [Approval History](./synara-agent-approval-history-contract.md) | `docs/contracts/synara-agent-approval-history-content.schema.json`                                     | Shared Core `agent_approval_history_live.rs`; `nativeAgentApprovalHistory.ts` | Future native approval history   | Writers emit canonical v1; readers fail closed on unsupported versions. Bounded summary only. |
