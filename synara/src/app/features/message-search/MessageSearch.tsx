...
export function MessageSearch({
  ...
  const rawGroups = useMemo(() => data?.pages.flatMap((result) => result.groups) ?? [], [data]);
  ...
  const handleSendersChange = (nextSenders?: string[]) => {
    setSearchParams((prevParams) => {
      const newParams = new URLSearchParams(prevParams);
      newParams.delete('senders');
      if (nextSenders && nextSenders.length > 0) {
        newParams.append('senders', encodeSearchParamValueArray(nextSenders));
      }
      return newParams;
    });
  };
