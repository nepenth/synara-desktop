...
const externalLinkProps: ComponentPropsWithoutRef<'a'> = {
  target: '_blank',
  rel: 'noreferrer noopener',
...
export const factoryRenderLinkifyWithMention = (
  mentionRender: (href: string) => React.JSX.Element | undefined
  ...
  const render: OptFn<(ir: IntermediateRepresentation) => any> = ({
    ...
    return (
      <a {...attributes} {...externalLinkProps}>
        {content}
      </a>
    );
  };
...
export const scaleSystemEmoji = (text: string): (string | React.JSX.Element)[] =>
  findAndReplace(
    text,
    EMOJI_REG_G,
    (match, pushIndex) => (
...
export function CodeBlock({
  ...
  const language =
    languageClass && languageClass.startsWith('language-')
      ...
      : languageClass;
  const plainText = useMemo(() => extractTextFromChildren(children), [children]);

  const LINE_LIMIT = 14;
  const largeCodeBlock = useMemo(() => plainText.split('\n').length > LINE_LIMIT, [plainText]);
  ...
  return (
    <Text size="T300" as="pre" className={css.CodeBlock}>
      ...
      <Scroll
        ...
        hideTrack
      >
        <div className={css.CodeBlockInternal}>
...
function PrismCode({
  ...
  const largeCode = children.length > PRISM_CHAR_LIMIT;
  ...
  return (
    <ErrorBoundary fallback={<code {...props}>{children}</code>}>
      <Suspense fallback={<code {...props}>{children}</code>}>
        <ReactPrism>
          {(ref) => (
            <code ref={ref} {...props} className={className}>
              {children}
            </code>
          )}
        </ReactPrism>
        ...
        if (name === 'code') {
          if (parent && 'name' in parent && parent.name === 'pre') {
            ...
            if (typeof codeReact === 'string') {
              ...
              return (
                <PrismCode props={props} className={lang}>
                  {codeReact}
                </PrismCode>
              );
            }
