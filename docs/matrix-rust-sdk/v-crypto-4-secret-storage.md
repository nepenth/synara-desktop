...
Recovery keys and passphrases are one-way IPC inputs. Each command owns its
input `String`, passes only a borrowed view to matrix-sdk, and zeroizes the
buffer immediately after the awaited operation. Status and operation responses
contain only booleans, enums, missing-secret categories, session generation,
operation outcome, and recovery-document save metadata.
