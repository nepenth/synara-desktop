...
| `CallWidgetDriver.ts:322-330` | `uploadFile(file)` delegates to `uploadCallWidgetFileWithNativeOwner` | **Native-first and fail-closed** for a logged-in native session; uses `matrix_upload_media`                                     | Upload owner closed by **#328**; legacy callback remains for non-native use |
...
The Rust commands live in `src-tauri/src/matrix/auth/product.rs` and are
registered and permissioned in the merged #407 slice. This docs truth-up does
not edit those product files.
