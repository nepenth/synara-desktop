...
Release-candidate and feature heads run CI and package smoke once, on the
pull request. `push` is reserved for `main` after merge and for manual
`workflow_dispatch`. Version/changelog/release-note diffs are **metadata-only** and skip iOS,
