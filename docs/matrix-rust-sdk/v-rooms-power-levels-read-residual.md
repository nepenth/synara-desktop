...
| Base tip | `f92a33f9` on `feature/matrix-rust-sdk-full-replacement` |
| Source | #437 read-residual content, refreshed after merged #405 and #439 |
...
| Serial handoff | **#446 extract → module fan-out → READ**; #446 owns the behavior-preserving `product.rs` extraction/split before READ product work |
...
>
> **Serial guard.** #446 owns the current serial `product.rs` extraction/split
> under [product-lane-protocol.md](product-lane-protocol.md). After that
> behavior-preserving extract lands, module fan-out may begin. Do not begin
> READ product implementation or register READ commands before the extract is
> complete and the READ module lane is explicitly released.

## 1. Residual boundary

This packet records the remaining **READ** work at `f92a33f9`:
...
At this tip, #439 is merged, but no READ snapshot command is present or claimed.
...
`f92a33f9`, and this packet does not authorize edits to `product.rs`.

Product implementation is serial-gated: #446 owns the current `product.rs`
extract; module fan-out follows #446; READ command registration and product
implementation wait for that sequence and an explicit READ-module handoff.
...
serial handoff above: **#446 extract, then module fan-out, then READ**. A
separate READ-module handoff is required before command registration or
frontend fan-out.

## 6. Done-when for this documentation packet

- The packet is based at `f92a33f9` and targets
...
- It incorporates #437's read inventory while recording #405 as merged and
  ...
  creator reads.
- It records merged #439's separate WRITE completion and makes the
  **#446 extract → module fan-out → READ** serial order explicit.
