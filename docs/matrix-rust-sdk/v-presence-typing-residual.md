...
| Measured tip  | `d82e043db25e4ec786bde103c4d457a898ef664b` on `feature/matrix-rust-sdk-full-replacement`                                  |
...
> **Post-merge note at `d82e043d`.** #458 is merged. The first presence slice
> lands the native snapshot/subscription route, profile binding, JavaScript
> presence-owner deletion, and focused local evidence. The rebase hold wording
> in #467 is superseded. The presence residual remains open because authenticated live
> proof and full acceptance evidence are **Not confirmed**; draft docs work is
> allowed, but this file is not a merge-readiness or completion claim. `main`,
> #39, `dual_backend`, and V-BURN remain out of scope or forbidden as stated.
...
- **The generic stream topic is still not the product route.** `PresenceIndex`
  is now reused by the live owner, while `PresenceStreamBody` and the generic
...
use this audit as evidence that the presence vertical, V-BURN, or the typing
residual is complete. V-BURN remains HOLD.
...
The remaining presence gap is evidence, not a second backend: no
authenticated two-client desktop proof with retained command/event readback is
recorded at `d82e043d`, and the full lifecycle/error acceptance matrix has not
been independently accepted. The generic `presence` stream topic and
...
| Presence acceptance       | Rerun the focused lifecycle/error/source-absence evidence at `d82e043d`, retain results, and complete independent acceptance. Local tests/source guards are not live proof.                                                                                          |
...
This document records the #458 first slice but does not claim presence
