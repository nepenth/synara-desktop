...
| Measured tip  | `b0fd42413a76b1000d1fab23ea225309ca62f82e` on `feature/matrix-rust-sdk-full-replacement`                                  |
...
- **CallWidget #407 is merged at this measured tip.** That merge does not add
  a presence owner or presence command; powers-bulk may be the next serial
  `product.rs` slice, so this presence packet remains docs-only and does not
...
| Local typing send/clear    | `src-tauri/src/matrix/auth/product.rs:2038-2048` — `matrix_typing_set` validates the active native session and calls `Room::typing_notice`; registered in `src-tauri/src/lib.rs:440-441` | `synara/src/app/hooks/useTypingStatusUpdater.ts:7-45` decides when to invoke native IPC, throttles sends, and schedules the local timeout; `RoomInput.tsx:250,630,687,810` drives it from compose events                     | Native network send; JS timing/orchestration remains                                                 |
