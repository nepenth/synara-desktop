...
| V-SEND.R-CALL-UPLOAD            | **#328** `CallWidgetDriver.uploadFile` → `uploadCallWidgetFileWithNativeOwner` / `matrix_upload_media` fail-closed; remaining widget JS reads/download are documented in [the residual inventory](v-send-call-widget-residual.md) |
...
- ~~**V-SEND.R-CALL-UPLOAD**~~ **#328** upload owner; remaining `getMediaConfig` / `downloadFile` / `getKnownRooms` are widget-adjacent JS residuals documented in [the inventory](v-send-call-widget-residual.md). ~~**V-SEND.R-GIF-PACK**~~ **NOOP** ([audit](v-send-gif-pack-audit.md)); composer/thumbnail legacy-web fallbacks — [upload call-site audit](v-send-upload-residual-audit.md); ~~**#325**~~ msgContent thumbnails via `uploadMediaNative`
