...
| V-SEND.R-PACK-READ              | **#297** snapshot; **#318** subscribe; **#320** room ids; **#365** delete JS pack `get*` helpers + web fallback (imports/allowlist **157**)                                                      |
...
| V-SEND.R-CALL-UPLOAD            | **#328** native upload; **#362** `getKnownRooms` native snapshot + media config/download fail-closed ([residual](v-send-call-widget-residual.md))                                                |
| Composer thumbnail (msgContent) | **#325** video thumbnails via `uploadMediaNative` / `matrix_upload_media` fail-closed                                                                                                            |
| Composer GIF/upload fallbacks   | **#363** delete `RoomInput` GIF + `msgContent` thumbnail JS `mx.uploadContent`/`mx.sendMessage` fallbacks; native-only fail-closed                                                               |
| Room leave vertical             | **#364** `matrix_room_leave` + LeaveRoom/LeaveSpace native owner; leave prompts drop js-sdk (imports/allowlist **157→155**)                                                                      |
...
| PR          | What                                                    |
| ----------- | ------------------------------------------------------- |
| (none open) | Product idle on tip `8577f0b7`. V-BURN HOLD. #39 gated. |

## Left (finish-line order)

1. **Room join/create lifecycle — native full vertical.** Leave is closed by
   **#364**. Re-home join/create UI through Tauri IPC to the live `matrix-sdk`,
   fail closed on native command absence/failure, and delete the superseded JS
   owner in the same slice. See [P6.9 room ops](p6.9-room-ops.md) and
   [P2.6 lifecycle](p2.6-destructive-lifecycle.md).
...
5. **CallWidget media-config/download native IPC.** Upload (#328) and
   `getKnownRooms` (#362) are closed; `getMediaConfig` / `downloadFile` fail
   closed until owning native IPC exists. See the
   [CallWidget residual inventory](v-send-call-widget-residual.md).
...
7. **V-BURN.2 — zero production importers.** Final repository-wide importer
   audit remains **left**; current production importers are **155**. See the
