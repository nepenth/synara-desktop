...
| CallWidget-adjacent, post-#359                            | `features/call-status/**` (4), `features/call/CallMemberCard.tsx`, `plugins/call/**` (3), `hooks/useCall.ts`, and `hooks/useCallEmbed.ts` — 10 importers                                                                                                                 | #328 closes the native upload owner, and #407 closes the native media-config/download owners with bounded validation and terminal failures. `CallWidgetDriver` remains an SDK-boundary importer; full call/widget cutover and V-BURN readiness are not claimed. See [the CallWidget residual](v-send-call-widget-residual.md). |
