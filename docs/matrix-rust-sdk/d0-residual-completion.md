...
| **V-SEND.4** | Emotes / notices / rich HTML + mentions | **In flight**: extend `matrix_send_text` with message type, formatted body, mentions, and reply; remove composer `mx.sendMessage` fallback for emote/notice                                                                                                                            | Product parity for retained composer features           |
