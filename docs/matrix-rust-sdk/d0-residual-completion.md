...
| Field                     | Value                                                                                                                                                                                         |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Status                    | **Active — tip `88ed143` (#250); serial #253 → #254 → #240 HOLD** (2026-07-31)                                                                                                                 |
| Policy                    | [full-vertical-policy.md](full-vertical-policy.md)                                                                                                                                            |
| Integration tip at policy | `0400306` (D0.1–D0.5 merged; D0.5 was **crypto minimum**)                                                                                                                                     |
| Current integration tip   | `88ed14308227b2eec2bed4fc33d97cfa0a2270f3` (V-SEND.3 #250 merge)                                                                                                                               |
| Active PRs                | Draft [#253](https://github.com/nepenth/synara-desktop/pull/253) V-SEND.4; draft [#254](https://github.com/nepenth/synara-desktop/pull/254) V-ROOMS.2b (dirty); [#240](https://github.com/nepenth/synara-desktop/pull/240) V-TIMELINE HOLD |
| Session handoff           | [SESSION-HANDOFF.md](SESSION-HANDOFF.md)                                                                                                                                                      |
...
| **V-ROOMS.2** | Spaces / hierarchy / lobby           | **2a DONE**; **2b** draft [#254](https://github.com/nepenth/synara-desktop/pull/254) (dirty vs tip; rebase after #253); writers/mutations may remain **2c**                                                | Native space hierarchy ownership (2a parent map + 2b lobby summaries)               |
...
| **V-ROOMS.2b** | Hierarchy summaries (lobby)          | **In flight** [#254](https://github.com/nepenth/synara-desktop/pull/254) head `71c2877`; dirty vs tip `88ed143`; claimed get_hierarchy ownership + `IHierarchyRoom` import deletion; rebase/CI required   | Native hierarchy summaries; JS getRoomHierarchy/`IHierarchyRoom` deleted            |
...
| **V-ROOMS.5r** | `m.direct` DM user list             | **DONE #252** at `9579ea4`; `matrix_mdirect_snapshot.userIds` (joined-filtered) owns `mDirectUsersAtom` / `useDirectUsers`; JS AccountData reader deleted; importers **187→187**                                                                     | Native user-list projection; JS useDirectUsers AccountData owner deleted            |

### V-TIMELINE — full timeline read vertical (D0.3 gaps)

| ID               | Capability                               | Residual today                                                                                                                                                                                                                                                                                                                                                                                   | Done when                                                                                       |
| ---------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| **V-TIMELINE.1** | Virtualized timeline on native DTOs      | Draft #240 at `bf9ac48` (required CI green / mergeable clean at handoff); withdraws incomplete shell and records cutover contract. **Presenter unselected**; do not delete `RoomTimeline.tsx` until full render/action/media route + runtime proof. Serial after #253/#254. | Complete all render/action/media paths and runtime proof before presenter selection or deletion |
...
| **V-SEND.3** | Polls                                   | **DONE #250** at `88ed143` (reviewed head `761d2ef`); Synapse native poll proof Confirmed; relation read residual remains in V-TIMELINE                                                                                                                                             | Native                                                  |
| **V-SEND.4** | Emotes / notices / rich HTML + mentions | **Active** [#253](https://github.com/nepenth/synara-desktop/pull/253) head `486b3d7` (on tip); Synapse rich-message proof green; Validate/attachment proof in progress at handoff                                                                                                      | Product parity for retained composer features           |
...
1. Treat tip `88ed143` (V-SEND.3 #250) as current; tip may move when #253 merges.
2. Finish **V-SEND.4** [#253](https://github.com/nepenth/synara-desktop/pull/253) serially (CI→undraft→`gh pr merge --merge`). Do not fight the owning worktree.
3. Rebase/fix/merge **V-ROOMS.2b** [#254](https://github.com/nepenth/synara-desktop/pull/254) onto the post-#253 tip.
4. Continue **V-TIMELINE** [#240](https://github.com/nepenth/synara-desktop/pull/240) contract gaps until closable. Required CI may already be green — that is **not** permission to select the presenter or delete `RoomTimeline.tsx`.
5. Continue remaining **V-AUTH** / **V-ROOMS** / **V-SEND** gaps, deleting each superseded JS owner in its slice.
6. **V-BURN** final convergence audit + npm dependency removal
