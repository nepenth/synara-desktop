...
| **V-AUTH.2** | Token login                    | **DONE** (product non-retention) | Desktop does **not** retain `m.login.token`; TokenLogin/SSO completion already deleted in V-AUTH.1; unused Rust `login_with_token` foundation removed; no `matrix_login_token` IPC; discovery may still parse HS-advertised token flows; focused negative/privacy tests; import delta **0** (JS owner already gone); see [v-auth-2-token-login.md](v-auth-2-token-login.md) |
...
| **V-ROOMS.2** | Spaces / hierarchy / lobby           | **2a DONE**; **2b hierarchy-summary read active**; local graph/mutations remain **2c** js                                                                                                                   | Native space hierarchy ownership across parent map, lobby reads, and mutations      |
...
| **V-ROOMS.2b** | Lobby hierarchy-summary reads      | **Candidate** from `9579ea4`; native typed hierarchy pagination replaces WebView `getRoomHierarchy`; files **187 / 200**, direct lines **249→244**; live proof unclaimed                                     | Native hierarchy summaries; JS hierarchy request/types deleted                      |
...
| **V-ROOMS.5r** | `m.direct` DM user list             | **DONE #252** at `9579ea4` (candidate `5b8de9d`); native users own autocomplete; JS AccountData reader deleted; importers **187→187**                                                                                                               | Native user-list projection; JS useDirectUsers AccountData owner deleted            |
...
| **V-SEND.4** | Emotes / notices / rich HTML + mentions | **In flight** [#253](https://github.com/nepenth/synara-desktop/pull/253): extend `matrix_send_text` with message type, formatted body, mentions, and reply; remove composer `mx.sendMessage` fallback for emote/notice                                                                 | Product parity for retained composer features           |
