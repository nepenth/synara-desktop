...
| **V-SEND.5** | Threads                                 | Draft: native composer text/attachment `m.thread` via `threadRoot` on `matrix_send_text` / `matrix_send_attachment` (see [v-send-5-threads.md](v-send-5-threads.md)); sticker/GIF/poll-in-thread + thread list/timeline remain residual | Composer text/attachment thread send owned natively; runtime proof Confirmed |
