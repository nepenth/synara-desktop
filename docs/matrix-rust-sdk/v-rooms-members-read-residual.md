...
| Field   | Value                                                                                                                                                                       |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Status  | **Active post-#405/#439 READ residual** — #395/#405 close native member enumeration; merged #439 closes the separate bulk WRITE, while power-level/creator READs remain residual |
| Tip SHA | `f92a33f9` (current feature tip; #405 landed at `176fc7c5`, #439 landed at this tip)                                                                                         |
| Base    | `feature/matrix-rust-sdk-full-replacement`                                                                                                                                  |
| Policy  | [full-vertical-policy.md](full-vertical-policy.md) — physical deletion inside each owning slice                                                                             |
| Follow-up | **Power-level/creator READ residual** — [READ packet](v-rooms-power-levels-read-residual.md); #446 owns the serial `product.rs` extract, then module fan-out |
| Related | **#375 and #439 merged** (moderation and bulk power WRITE verticals), P4.6 members index, [p4.6-members.md](p4.6-members.md), [p4.3-membership-unread.md](p4.3-membership-unread.md) |
...
> slice. This docs-only packet is based at `f92a33f9`; #405 is merged at
> `176fc7c5`, and #439's bulk power/tag WRITE is merged at this tip. It does
> not modify product code or `product.rs`; the next serial product operation
> belongs to the behavior-preserving **#446 extract**, followed by module
> fan-out before READ implementation. No cutover; `dual_backend=false` and
> V-BURN remains **HOLD**.
...
The serial product lane is now **#446 extract → module fan-out → READ**. #446
owns the behavior-preserving extraction/split of `product.rs`; no READ command
registration or READ product implementation starts before that lane is released.
...
This is a scoped first slice with `dual_backend=false`: native session does not fall
...
| Surface | Current tip (`f92a33f9`) | Residual after merged #405/#439 |
| ------- | ------------------------ | -------------------------- |
...
This is the current post-#405/#439 truth: drawer/lobby/mentions member reads
are closed and bulk power/tag writes are landed; power-level/creator reads are
not. A native desktop session must never use the legacy member path as a
fallback, and `dual_backend=false` remains the explicit policy state.
...
| Product input                                                        | **#405 merged** at `176fc7c5`; drawer/lobby/mentions member wiring is landed. **#439** bulk powers/tag WRITE is merged at `f92a33f9`; power/creator READs are not included. |
| Product changes in this PR                                           | None; this packet is docs-only, based at `f92a33f9`, and does not edit `product.rs` |
...
`useRoomMembers`. At tip `f92a33f9`, I confirmed the live native producer in
