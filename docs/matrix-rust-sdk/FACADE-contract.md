...
| sendStateEvent | **20** | matrix_set_room_* + GAP for arbitrary state event types |
...
| matrixRTC | **8** | native call (matrix-call already native) — keep |
...
| F7 | `0` lockdown + dep removal + V-BURN checklist | npm dep removal, allowlist 0, full green, audit | **0** |
...
## 6. Sources
- `inventory-matrix-sdk-usage` at tip: 1 production + 0 test import files; allowlist 1.
