...
- PR #1152 passed its required Quality gate and merged to main as `d56110e4`.
  ...
  `release/v2.1.39` candidate. Stacked PRs #1129–#1149 stay unmerged leftovers.
- Desktop Node validation, Rust desktop shell, iOS compile, package smoke, and
  ...
  installers.
- Remaining human checks before tagging: date-rail hover in a long room;
...
| MAC-DESK-007a | Agent approval native actions       | Approve once / Deny from a fresh agent-approval OS notification passes through `matrix_agent_approval_decide`, which resolves the event, enforces Hermes's five-minute expiry, ignores Hermes's bot-owned seed reactions, and sends at most one current-account decision; approve-always is not offered; the default click opens Review; in-app approval cards retain bounded prompt context and explicit approve-always confirmation. Stale/expired/unresolved or already-decided prompts do not send. | Action ids used, event age, shared-core status, pass/fail. |
