...
| Item | Verified state |
|---|---|
| Feature evidence tip | `feature/shared-native-core` is `9bf4aa2c`, the merge commit for #852. |
| Immediately preceding merges | #851 docs after #850, #852 `matrix_verification_start` through Core::command. |
...
anchor, but it is no longer the feature-tip provenance; use `9bf4aa2c`/#852 for
...
### P2 — in progress: thirty registered commands

The Core registry registers exactly these thirty names:
