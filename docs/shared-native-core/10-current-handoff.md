...
| Item | Verified state |
|---|---|
| Feature evidence tip | `feature/shared-native-core` is `faf5f28c`, the merge commit for #840. |
| Immediately preceding merges | #839 docs after #838, #840 `matrix_verification_accept` through Core::command. |
...
anchor, but it is no longer the feature-tip provenance; use `faf5f28c`/#840 for
...
### P2 — in progress: twenty-four registered commands

The Core registry registers exactly these twenty-four names:
