...
| Item | Verified state |
|---|---|
| Feature evidence tip | `feature/shared-native-core` is `f003d8f7`, the merge commit for #832. |
| Immediately preceding merges | #831 docs after #830, #832 image-pack writes through Core::command. |
...
anchor, but it is no longer the feature-tip provenance; use `f003d8f7`/#832 for
...
### P2 — in progress: nineteen registered commands

The Core registry registers exactly these nineteen names:
