...
| Item | Verified state |
|---|---|
| Feature evidence tip | `feature/shared-native-core` is `fecf19cd`, the merge commit for #874. |
| Immediately preceding merges | #873 docs after #872, #874 timeline forward text/media. |
...
anchor, but it is no longer the feature-tip provenance; use `fecf19cd`/#874 for
...
### P2 — in progress: fifty registered commands

The Core registry registers exactly these fifty names:
