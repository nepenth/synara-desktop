...
At `feature/shared-native-core` `9f12bc38`
(#830, after #829), P2 registers sixteen names—the prior fourteen plus
`matrix_get_user_image_pack` and `matrix_get_room_image_packs`. The previous
fourteen remain—
...
`matrix_room_join_rule_snapshot`, and `matrix_get_global_image_packs`—and all
