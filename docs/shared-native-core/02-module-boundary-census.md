...
`b26b2a4b` (#755, after #754).

## 2.1 Desktop application-logic layer

The P0 census counted **285 `.rs` files** under `src-tauri/src/matrix/`. At the
current evidence tip, there are 113 tracked Rust files under that desktop path
and 189 under `crates/synara-core/src/app/`; the difference reflects P1 moves,
...
store identity/paths/key-material/vault-trait, client-builder error/features, lifecycle
recovery-copy / remote-logout policy enums / lifecycle error, auth device-name, and auth
