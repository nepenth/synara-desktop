...
`18b5c647` (#758, after #757).

## 2.1 Desktop application-logic layer

The P0 census counted **285 `.rs` files** under `src-tauri/src/matrix/`. At the
current evidence tip, there are 109 tracked Rust files under that desktop path
and 193 under `crates/synara-core/src/app/`; the difference reflects P1 moves,
...
lifecycle recovery-copy / remote-logout / wipe / error, auth device-name, and auth
