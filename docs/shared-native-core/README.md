...
| Evidence base | `feature/shared-native-core` at `00e90a2f` (#912, after #911; prior bounded P4 evidence remains #708/#710) |
...
- **P2 is a partial transport registry, not the complete command migration.**
  ...
  desktop-census commands: the prior ninety-nine plus `matrix_backup_status`
  (#912). Setup/restore/repair stay desktop so secrets never cross the
  envelope. Attachment send stays desktop because bytes can exceed the 1
  MiB Core envelope. The rest of the census remains unregistered and
  fail-closed.
...
`00e90a2f` (#912, after #911). #708 and #710
