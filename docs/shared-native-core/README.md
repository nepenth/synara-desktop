...
| Evidence base | `main` at `76f67441` (#1006 merge of desktop JS media retire; parents `3cea3282` #1005 + `41c655f7` product tip). #1001 remains the P4-S12–S37 merge. #991 remains the feature-lane integration. |
...
`76f67441` (#1006). #1001 remains the P4-S12–S37 merge. #991 remains
the feature-lane integration.
...
#982 records local UniFFI generate. #983 recorded a temporary S10
leftover **stop** after a playbook §9.6 gate; that stop was
superseded by operator authorization and #986. #984 lands the S11
...
code. Read [10-current-handoff.md](10-current-handoff.md) for `main`
provenance, remaining gaps, and non-authorizations. Those files do not
...
- **Native ownership stays explicit.** Desktop remains the sole owner of its
  ...
  session, Keychain, store, crypto, sync, and lifecycle ownership no
  longer go through a product `MatrixRustSDKService`; they go through
  caller-owned SharedCore leftover/product adapters that fail closed
  without a live homeserver and do not start SyncService.
