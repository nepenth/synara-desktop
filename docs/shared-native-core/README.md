...
| Evidence base | `feature/shared-native-core` at `c51ab072` (#906, after #905; prior bounded P4 evidence remains #708/#710) |
...
- **P2 is a partial transport registry, not the complete command migration.**
  `Core::command` has typed envelopes and currently registers ninety-one
  desktop-census commands: the prior eighty-nine plus
  `matrix_send_poll` / `matrix_poll_respond` (#906). Attachment send stays
  desktop because bytes can exceed the 1 MiB Core envelope. The rest of the
...
`c51ab072` (#906, after #905). #708 and #710
