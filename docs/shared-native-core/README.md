...
| Evidence base | `feature/shared-native-core` at `c9a315ea` (#916, after #915; prior bounded P4 evidence remains #708/#710) |
...
- **P2 is a partial transport registry, not the complete command migration.**
  ...
  three desktop-census commands: the prior one hundred one plus
  `matrix_room_directory_search` / `matrix_room_directory_cancel` (#916).
  Setup/restore/repair stay desktop so secrets never cross the envelope.
  Attachment send stays desktop because bytes can exceed the 1 MiB Core
  envelope. The rest of the census remains unregistered and fail-closed.
...
`c9a315ea` (#916, after #915). #708 and #710
