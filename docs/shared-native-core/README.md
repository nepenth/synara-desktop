...
| Evidence base | `origin/feature/shared-native-core` at `78028d6` (`feat(core): route crypto status through shared transport`, #698) |
...
- **P2 is a partial transport registry, not the complete command migration.**
  `Core::command` has typed envelopes and currently registers exactly five
  desktop-census commands: `matrix_login_flows`, `matrix_register_flows`,
  `matrix_session_snapshot`, `matrix_sync_status`, and `matrix_crypto_status`.
  The session snapshot plus the payload-free sync- and crypto-status handlers
  are present; the rest of the census remains unregistered and fail-closed.
- **P3 has a bounded desktop seam.** Existing Tauri commands route the two
  ...
  React-facing DTOs. This is not the planned whole-shell swap: the other
...
- **P4 has a project-owned UniFFI scaffold, credential-free typed login-flow
  discovery, iOS homeserver-discovery use, and an XCTest that calls the
  generated Rust FFI scaffold.** It has not migrated the iOS session, room-list,
...
The following describes only merged source reachable from `78028d6`.
...
| P2 | #683–#689, #694, #698 | The five-command registry above is live; it is not parity with the full desktop invoke census. |
| P3 | #690–#691, #694, #698 | The listed auth and read-only/session bridges use Core; `src-tauri` is not yet a fully thin shell. |
| P4 | #685, #692–#693, #696 | UniFFI scaffold and bounded login discovery/link coverage exist; service migration and Apple release proof do not. |
| P5 | None | Not started. The release gates below remain required. |

### Unmerged work is not current program state

[PR #699](https://github.com/nepenth/synara-desktop/pull/699),
`feat(ios): mirror safe Core session projection`, is **open and unmerged** at
this evidence base. Its proposed bounded `SessionProjectionCore` mirror is not
part of `78028d6`; do not count it as P4 completion, iOS migration, or P5
progress. It remains future work unless and until it lands separately.
