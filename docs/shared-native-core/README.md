...
| Evidence base | `feature/shared-native-core` at `b3ed8891` (#922, after #921; prior bounded P4 evidence remains #708/#710) |
...
- **P2 is a partial transport registry, not the complete command migration.**
  ...
  six desktop-census commands: the prior one hundred five plus
  `matrix_room_list_snapshot` (#922). Invite snapshot stays desktop because
  it mutates `invite_avatars`. Password continuation stays desktop so the
  credential never crosses the envelope. Export/import stay desktop so
...
`b3ed8891` (#922, after #921). #708 and #710
