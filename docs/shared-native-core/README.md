...
| Evidence base | `feature/shared-native-core` at `b72a2f9a` (#914, after #913; prior bounded P4 evidence remains #708/#710) |
...
- **P2 is a partial transport registry, not the complete command migration.**
  ...
  one desktop-census commands: the prior one hundred plus
  `matrix_room_directory_protocols` (#914). Directory search/cancel stay
  desktop (request-authority registry). Setup/restore/repair stay desktop
  so secrets never cross the envelope. Attachment send stays desktop
  because bytes can exceed the 1 MiB Core envelope. The rest of the census
...
`b72a2f9a` (#914, after #913). #708 and #710
