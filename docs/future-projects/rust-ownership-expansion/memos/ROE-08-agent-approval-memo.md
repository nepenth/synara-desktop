...
themselves. `!approve session` appears only in reply-instruction text; no
client implements a session-approval action.
...
| Expiry / freshness | **Core authority for writes.** 5-minute event age, 60s future skew, `>=` TTL. | Duplicate constants: `AGENT_APPROVAL_NATIVE_ACTION_TTL_MS`, `isAgentApprovalNativeActionExpired` (uses `>` and optional notification-created clock). Used for recent-event notification *emission* (`RECENT_AGENT_APPROVAL_MS`) and planner tests; not the live OS write gate. | `SynaraNotificationActionContract.nativeActionTTL` is unused in the live planner (`now _:`). `SynaraAgentApprovalFreshness.isFresh` (5 min, 60s future, `origin > 0`) gates NSE time-sensitive category / interruption only. | Core planner tests; desktop `isAgentApprovalNativeActionExpired` tests; iOS `NotificationPreviewSupportTests.swift`. |
...
| Room / sender / power / spoofing | Exact event resolve; current-account reactions only; no extra sender-trust policy. First-line heading is the spoofing control. | Notification emit skips the current user. Card send honors `canSendReaction` (room power observation). Classifier `includes` is the weaker spoofing rule. | Same: detector `contains`; NSE restores category only after Core classification (`docs/agent-approval-notification-proxy-spec.md`). Proxy category/kind cannot grant controls. | Core heading-negative tests; NSE `NotificationService.swift`. |
...
1. **No ownership change.** Keep platform detectors for cards and desktop
   notification emission. Keep TypeScript/Swift planners as routing and local
   dedupe. Leave in-app once/deny on `reaction_ensure`. Falsified if a live OS
   path still sends a decision reaction without `decide_agent_approval`, or if
   a Hermes fixture shows a platform-accepted prompt that Core would refuse
   *and* that refusal is required to prevent a successful unauthorized
   approval (not only a fail-closed notification).

2. **Bounded extraction (recognition only).** Make Core
   `is_agent_approval_prompt` the sole eligibility owner for “this is an
   approval prompt.” Presenters keep command/reason/source parsing and all UI.
   OS planners stay routing-only. No new approval subsystem. Falsified if
   first-line Core and platform `includes` agree on the real Hermes heading
   corpus *and* no desktop approval notification or card is shown for a
   Core-negative body.

3. **Broader Core model.** Also force in-app once/deny through
   `decide_agent_approval`, project a row-level approval flag, and delete
   platform planners. Falsified if in-app once/deny must remain able to decide
   after the five-minute OS window (the in-app contract Core exists to
   protect), or if a row flag would move card layout or OS delivery into Core.

Stay-put is the default unless harmful duplicated *authority* is proven. The
proven harm is eligibility drift (cards and desktop approval notifications on
Core-negative bodies), not a second Matrix write engine on the OS path.

## Recommendation

**Extract a bounded subset:** shared prompt *recognition* (eligibility), not
cards, not OS delivery, not a new planner.

Confidence: medium. Native OS writes are already correctly owned. The leftover
that still competes with `agent_approvals.rs` is the pair of platform
classifiers (`detectAgentApprovalPrompt` / `detectAgentApprovalPromptBody` and
`SynaraAgentApprovalPromptDetector.detect`), plus in-app once/deny
`reaction_ensure` skipping Core prompt/expiry/already-decided checks. The
TypeScript and Swift *planners* duplicate constants and action-id maps but are
not the Matrix write owner; they are observation and navigation gates.

That classifier duplication **is competing authority** for eligibility. It is
not competing write authority for OS approve-once/deny. Structured
...
Strongest objection (stay-put): Core already owns the only write path that
could bypass the in-app approval contract; remaining TypeScript/Swift detectors
are presentation and notification-emission helpers whose false positives fail
closed on `decide_agent_approval`; unifying recognition now would still need a
presenter-visible signal and would start product work while the goal graph is
stopped on the P4 engine-ready gate.

Unresolved questions:

- Whether in-app once/deny after five minutes is required product behavior or
  an accidental second policy.
...
- Whether `!approve session` is a future Hermes action or documentation-only
  text.
...
**STOP.** This extract recommendation is a research stop, not an
implementation start. A human must accept the recommendation and, if the
accepted boundary still matches ADR 0004, decide whether any product change is
chartered at all. The current goal graph does not permit a new residual
implementation slice: P4 engine-ready is pending/blocked; do not invent S38;
do not start P5. No implementation plan, Core API, UniFFI change, or product
edit follows from this memo.
