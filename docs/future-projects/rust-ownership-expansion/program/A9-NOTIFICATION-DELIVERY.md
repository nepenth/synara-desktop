...
not confirmed. Desktop tray delivery through the shipped Core→renderer
observation stream is confirmed live on macOS at `466e41e` with a receipt that
lands after Notification Center records the delivery; a live `failed` receipt
under a denied permission could not be reproduced because the OS did not
refuse the unsigned harness bundle (see the 2026-09-08 desktop findings
below). Linux tray delivery is not confirmed.
...
| (g) DM after both identities were set to `allow: NO`            | observed once; `show`, `sound true`                                       | `delivered` 53 ms; OS still logged `Delivering` | `3 / 0 / 0`                                    |

`usernoted` logged a `Delivering <NotificationRecord …> to [.alert .lockScreen
.notificationCenter]` record about 50 ms after each `Record` for (a)–(c), (e),
(f), and (g), matching the receipt timings above; the receipt now lands after
the OS record exists rather than at spawn time. Identity: runs (a)–(e) used
the unbundled debug binary and registered as `com.apple.Terminal` (the
`tauri::is_dev()` choice); runs (f) and (g) placed the same binary inside the
throwaway bundle and Notification Center registered the connection under the
bundle's own identifier, so a bundled product build presents under its own
identity.
...
- Deny notification permission → ack arrives as `failed`, counted once, no
  repeat: **Not reproducible on this rig; the OS did not refuse.** The
  `usernoted` settings dump attached to every record above reads
  `authorizationStatus: Denied` for the sending identity (Terminal for the
  unbundled runs; the operator confirmed Terminal notifications were already
  off, and the a9qa identity was new and unauthorized for the bundled run).
  At 15:52:34–35 the operator's System Settings visit dismissed both pending
  permission prompts and the log recorded `Authorization set … to allow: NO`
  for `com.apple.Terminal` and for the a9qa identity. A DM sent afterwards
  (case (g), 15:53:02) was still processed as `Delivering … to [.alert
.lockScreen .notificationCenter]`, appeared in `deliveredNotifications`, and
  the receipt returned `delivered` in 53 ms; the operator had seen banners
  earlier in the run under the same `Denied` status. On macOS 26.6.2 the
  legacy `NSUserNotificationCenter` delivery from this unsigned harness
  bundle is therefore not refused by a denied authorization, so there was no
  refusal for the receipt to observe. The receipt reported what Notification
  Center reported. Whether the signed product bundle (which gets a real entry
  under System Settings → Notifications; the throwaway bundle never did) is
  refused when its switch is off, and whether the receipt then times out to
  `failed`, remains **Not confirmed** and needs a signed build.
...
| desktop delivery receipt reaches Core                    | `matrix_notification_dismiss` closed `outcome`; per-session ledger counted once per released candidate; renderer awaits the OS answer and never retries; SDK-path proof for undecryptable encrypted-room events and focus changes           | `delivered` counted live on macOS (1→2→3→4, `failed 0`) at `466e41e` with the receipt landing after the OS record (52–64 ms); refused-delivery `failed` receipt deterministic only; live deny **Not reproducible** on the unsigned harness (OS delivered under `authorizationStatus: Denied`), signed-bundle deny **Not confirmed** |
