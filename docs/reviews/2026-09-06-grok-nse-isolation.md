...
Follow-up on `cursor/client-reliability-integration-d7fa`: the two wiring P2s are applied. `ci-build.sh` now captures completed `nm` output and fails on a non-zero inspection before searching the buffer; isolation and Core scaffold checkers require the production-feature invocation and the three Apple triples. A `[target.*.build-dependencies]` fixture was attempted but `cargo tree -i synara-core` does not surface those edges even with an explicit `full-uniffi` feature and a `build.rs`; host `[build-dependencies]` already fail isolation. That remaining P2 stays a Cargo-tree invert limitation, not a dropped product guard.
