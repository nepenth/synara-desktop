...
The existing UniFFI pusher-registration deadline remains five seconds because logout waits for reconciliation and Swift cancellation does not cancel that UniFFI future. A first installation now requires authoritative GET, policy PUT, confirming GET and pusher SET (repair of a disabled or misplaced policy adds one operation). A confirmed policy requires GET and SET. Loopback route tests establish ordering and failure behavior, not performance over a slow mobile connection. Timeout does not authorize a success claim or delete an existing pusher. This branch adds no independent retry loop or deadline extension. Migration is triggered by normal APNs registration after process launch; it does not update an app that remains terminated.
...
- The first executable CI run exposed an HTTP fixture defect: restoring the real SDK starts recovery initialization, which performs an independent default-secret-key account-data read. The fixture mistakenly consumed that request as the next policy operation. A local diagnostic run captured the exact unexpected read; modeling one initialization read exposed the second read from the backup-state observer. The final fixture routes only that exact authenticated, bodyless endpoint (bounded to two reads) and awaits the SDK initialization boundary. Every policy method, path, body, count and ordering assertion remains strict.
- Current-source local clean rerun: all 10 edit-policy tests passed, including all six HTTP owner-route scenarios. The complete Core library suite passed: 882 tests, 3 ignored, zero failures. Tests used the pinned repository `1.93` toolchain; formatting used exact Rust 1.93.0. The initial shared-target binary had zero matching policy tests and was rejected as evidence; only package artifacts were invalidated before the current-worktree rebuild.
- Existing account-bound pusher integration suite passed all six tests, including retained credentials across account rotation and idempotent cleanup. Remote CI must still be rerun on the final commit. No live account push rules were modified by the diagnostic API probe.
