...
| Medium | `src-tauri/capabilities/main.json` | The main webview may invoke login, secret-storage unlock, room-key import/export, media save, and clipboard read. A renderer bug is equivalent to the logged-in account for those commands. Update install and process restart are now a macOS-only capability. `opener:allow-open-url` is no longer granted; Rust still opens filtered URLs. | Split a smaller UI capability from secret and file commands. |
...
| Info | iOS `Info.plist` files | `ITSAppUsesNonExemptEncryption` is false on an E2EE client. ATS exceptions are absent. | Correct the export-compliance declaration before any App Store submission. |
