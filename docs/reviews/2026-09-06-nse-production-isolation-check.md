...
## Apple-target query correction after independent review

The first checker still used Cargo's host graph. Although direct Apple graph
and archive readback above established that the current product is narrow,
the checker could miss a future `target_os = "ios"` production feature leak.
Independent review reproduced that omission, so that checker revision is not
accepted as the preservation mechanism.

The checker now queries each target supported by the NSE generator:
`aarch64-apple-ios`, `aarch64-apple-ios-sim`, and `x86_64-apple-ios`. All retain
normal/build edges plus features and exclude dev edges. Each query must succeed
and exclude full-uniffi. These are metadata queries and do not require installed
Rust target libraries.

Four new actual-Cargo regressions first failed against the host-only checker:
an iOS-only cfg dependency and an isolated production leak on each supported
Apple triple all passed incorrectly while their macOS graphs were narrow.
After the target correction, all eight feature regressions pass, preserving
the existing dev-only success, normal/build leakage rejection, and failed-query
rejection. The actual repository checker passes all three Apple targets, and
the exact CI feature/archive block passes against the previously generated,
unchanged NSE archive. No Rust/Swift source or generated bytes changed, so no
heavy rebuild was needed. Evidence:
`/tmp/synara-nse-isolation-target-baseline.log`,
`/tmp/synara-nse-isolation-target-tests.log`, and
`/tmp/synara-nse-isolation-ci-target-block.log`.
