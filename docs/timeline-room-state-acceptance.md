...
These budgets are release gates for the modern timeline, read-state, and Recent
24h paths. Measurements must use release-like builds, deterministic fixtures,
...
| Recent entry                    | Within 1 second of the client processing the live sync event                                  |
| Recent expiry                   | Within 60 seconds of the exact 24-hour boundary                                               |
...
- Recent coverage includes local echo, remote echo, encrypted activity,
  decryption, reaction/edit/redaction non-bumps, membership change, reconnect,
  clock/time-zone change, and expiry without app restart.
