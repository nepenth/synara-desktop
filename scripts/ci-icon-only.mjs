...
const iconInfrastructurePatterns = [
  /^\.github\/workflows\/(?:ci|desktop-package-smoke)\.yml$/,
...
export const isIconOnlyChange = (files) => {
  ...
  const hasProductionIcon = names.some((name) => matches(productionIconPatterns, name));
...
const main = () => {
  ...
  const files = fromArgs.length > 0 ? fromArgs : readFileSync(0, "utf8").split(/\r?\n/);
