...
const sharedCoreEditMessage = readFileSync(
  resolve(root, "synara-ios/Synara/Services/SharedCoreEditMessage.swift"),
  "utf8"
);
const sharedCorePollRespond = readFileSync(
...
const sharedCoreTimelinePin = readFileSync(
...
const assertions = [
  ...
  [sharedCoreEditMessage, "editMessage", "P4-S9-25 product edit-message helper"],
  [sharedCoreEditMessage, "core: SharedCore", "P4-S9-25 helper takes an already-constructed SharedCore"],
  ...
  [sharedCorePollRespond, "pollRespond", "P4-S9-26 product poll-respond helper"],
  ...
  [swiftBindingsTests, "testSharedCoreTimelinePinWithoutSessionFailsClosed", "Swift P4-S9-28 fail-closed timeline-pin test"],
  [sharedCoreTimelinePin, "timelinePin", "P4-S9-28 product timeline-pin helper"],
  [sharedCoreTimelinePin, "timelineUnpin", "P4-S9-28 product timeline-unpin helper"],
...
for (const required of ["edit_message("]) {
  if (!sharedCoreBody.includes(required)) {
    throw new Error(`P4-S9-25 SharedCore must expose ${required}`);
  }
}
for (const required of ["poll_respond("]) {
  if (!sharedCoreBody.includes(required)) {
...
for (const required of ["timeline_pin(", "timeline_unpin("]) {
  if (!sharedCoreBody.includes(required)) {
...
if (sharedCoreEditMessage.includes("SharedCore(store:")) {
  throw new Error("P4-S9-25 helper must not construct-and-drop SharedCore");
}
if (sharedCoreEditMessage.includes("SharedCore.newWithSecretStore") || sharedCoreEditMessage.includes("newWithSecretStore")) {
  throw new Error("P4-S9-25 helper must not construct SharedCore");
}
for (const forbidden of ["sendText", "sendSticker", "sendPoll", "pollRespond", "backupStatus"]) {
  if (sharedCoreEditMessage.includes(forbidden)) {
    throw new Error(`P4-S9-25 helper must not wrap ${forbidden}`);
  }
}
if (sharedCorePollRespond.includes("SharedCore(store:")) {
  throw new Error("P4-S9-26 helper must not construct-and-drop SharedCore");
}
if (sharedCorePollRespond.includes("SharedCore.newWithSecretStore") || sharedCorePollRespond.includes("newWithSecretStore")) {
  throw new Error("P4-S9-26 helper must not construct SharedCore");
}
...
if (sharedCoreTimelinePin.includes("SharedCore(store:")) {
  throw new Error("P4-S9-28 helper must not construct-and-drop SharedCore");
}
if (sharedCoreTimelinePin.includes("SharedCore.newWithSecretStore") || sharedCoreTimelinePin.includes("newWithSecretStore")) {
  throw new Error("P4-S9-28 helper must not construct SharedCore");
}
