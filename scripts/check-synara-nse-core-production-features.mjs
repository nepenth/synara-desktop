...
// Cargo's per-target tree can omit a cfg(target_os="ios") build dependency
// because build dependencies run on the host. The all-target tree exposes that
// edge while `-e normal,build,features` still excludes dev-only fixtures.
