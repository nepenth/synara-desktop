...
test("repository inventory includes tooling dynamic import and records the current deletion delta", () => {
  ...
  assert.equal(
    ...
    172,
    `expected 172 desktop runtime production import files, found ${baseline.productionImportFiles}`
  );
  assert.equal(
  ...
  assert.equal(baseline.buckets.hook, 47);
  assert.equal(baseline.buckets.component, 29);
  assert.equal(baseline.buckets.page, 10);
  assert.equal(baseline.buckets.utility, 13);
