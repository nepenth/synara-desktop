...
test("repository inventory includes tooling dynamic import and records the current deletion delta", () => {
  ...
  assert.equal(
    ...
    0,
    `expected 0 desktop runtime production import files (F6c-2b drop), found ${baseline.productionImportFiles}`,
  );
  assert.equal(productionImportDeclarations, 0);
  assert.equal(
    baseline.testImportFiles,
    0,
    `expected 0 desktop runtime test import files, found ${baseline.testImportFiles}`,
  );
  assert.equal(baseline.buckets.feature ?? 0, 0);
  assert.equal(baseline.buckets.hook ?? 0, 0);
  assert.equal(baseline.buckets.component ?? 0, 0);
  assert.equal(baseline.buckets.page ?? 0, 0);
  assert.equal(baseline.buckets.utility ?? 0, 0);
  assert.equal(baseline.buckets.state ?? 0, 0);
  assert.equal(baseline.buckets.plugin ?? 0, 0);
  assert.equal(baseline.buckets["client-lifecycle"] ?? 0, 0);
