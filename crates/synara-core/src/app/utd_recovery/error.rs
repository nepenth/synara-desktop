...
impl std::fmt::Display for UtdRecoveryError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Invalid { diagnostic_id } => {
                write!(f, "invalid UTD recovery operation ({diagnostic_id})")
            }
            Self::NotFound { diagnostic_id } => {
                write!(f, "UTD recovery target not found ({diagnostic_id})")
            }
            Self::StaleGeneration {
                ...
                f,
                "stale UTD recovery generation ({diagnostic_id}): expected {expected}, observed {observed}"
            ),
        }
    }
}

impl std::error::Error for UtdRecoveryError {}
