...
2. Modularize `desktop.rs`. **Status:** In progress; first slices extracted external-link, session-base, and agent URL policy helpers into `src-tauri/src/desktop_url.rs`, shared text/route sanitization helpers into `src-tauri/src/desktop_sanitize.rs`, and file-transfer policy helpers into `src-tauri/src/desktop_file_transfer.rs`, all with direct Rust tests. `desktop.rs` is now 4887 LOC.
